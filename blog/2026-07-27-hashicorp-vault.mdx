...
All five functions share the same five antipatterns, but the risk is not uniform. `handleLoginRequest` and `handleRequest` are co-located in `vault/request_handling.go`, both touched 4 times in 30 days, and their bug-fix rate of 50% means recent activity on that file has included bug-fix commits. I'd treat those two as a single review unit — any change to one should prompt a look at the other.
