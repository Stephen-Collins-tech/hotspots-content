---
title: "MockingBird's training layer carries the highest structural debt — 5 functions to address first"
...
This is the top-ranked function in the repository, with an activity_risk of 17.28. The source excerpt makes clear why: `train` owns the entire Tacotron training lifecycle in a single function — directory scaffolding, device detection, GPU batch-size validation, model instantiation with a full hyperparameter spread, optimizer setup, and checkpoint loading. That's before the training loop itself begins.
