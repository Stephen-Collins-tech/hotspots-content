...
description: "lerna's top activity-risk hotspots are concentrated in libs/core (oidc, vendored fixtures) and the version/publish command initializers — all flagged as god functions with high cyclomatic complexity and structural debt that hasn't been touched in 127 days."
introContent: "lerna's oidc function hits CC 35 — structurally complex with a high blast radius when next changed. At 127 days untouched, it's overdue for refactoring before the next development push on the authentication layer."
pubDate: 2026-03-11
