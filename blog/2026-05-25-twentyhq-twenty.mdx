---
title: "twentyhq/twenty's sandbox and rendering layer carry the highest activity risk — 2 functions to address first"
...
### `assertIsDefinedOrThrow` — `packages/twenty-shared/src/utils/validation/assertIsDefinedOrThrow.ts`

This utility sits in the "watch" quadrant with a moderate activity-weighted risk score of 9.4 — structurally simple (CC 4, nesting depth 1, fan-out 1) but recently active with one touch in the last 30 days. Its name and location in a shared validation utilities package suggest it is a widely-called assertion helper used throughout the codebase to guard against undefined values. The structural metrics are fine; what warrants attention is *breadth of use*. If the behavior or signature of this function changes, every callsite in the shared package is affected. No action is needed for complexity, but any future changes here should be reviewed with callsite impact in mind.

### `applyRowLevelPermissionPredicates` — `packages/twenty-server/src/engine/twenty-orm/repository/workspace-select-query-builder.ts`

Also in the "watch" quadrant, `applyRowLevelPermissionPredicates` carries an activity-weighted risk score of 7.6 with CC 5, nesting depth 1, and fan-out 2 — structurally lean for what its name implies. The function appears to inject row-level security predicates into workspace query construction, a security-critical code path where correctness matters more than complexity score. It was touched three days ago, suggesting active development of the permissions layer. The low structural complexity is reassuring; the recommendation here is not refactoring but rigorous review discipline — permission predicate logic should have explicit test coverage for edge cases like empty predicate sets, conflicting rules, and cross-workspace queries, regardless of how simple the current implementation looks.
...
- **The permissions layer (`applyRowLevelPermissionPredicates`) deserves test coverage investment, not complexity reduction.** It is structurally simple but security-critical and actively changing — the right intervention is a comprehensive test suite, not a refactor.
