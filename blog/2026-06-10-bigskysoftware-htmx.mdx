---
title: "htmx's request and swap layer carries the highest structural debt — 5 functions to address first"
...
The file-level external signals add important historical context. `src/htmx.js` has 4 bug-linked commits, a bug-fix fraction of 0.27 (more than one in four commits to this file has been a bug fix), and a PR review comment density of 11.14 — the highest in the top five. Five distinct authors have touched the file in the last 90 days. None of that proves `issueAjaxRequest` specifically caused those fixes, but a function with CC 76 sitting in a file with that defect history warrants careful attention before the next change.
...
This function shares the same file-level external signals as `issueAjaxRequest`: 4 bug-linked commits, a 0.27 bug-fix fraction, and a PR review comment density of 11.14. It has not been touched in 44 days. The `exit_heavy` pattern is visible throughout — seven of the `switch` cases are early returns, and the default branch has its own early return when an extension handles the swap. That many exit points makes it difficult to write a test that exercises the settle-task path without also verifying correct short-circuit behaviour for all the cases above it.
