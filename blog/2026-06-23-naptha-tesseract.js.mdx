---
title: "tesseract.js's worker-script carries the highest activity risk — 4 functions to address first"
...
qaSection:
  ...
  - question: "Is tesseract.js actively maintained?"
    answer: "Yes, based on the data at commit a1ca80d. Four of the five top-risk functions — `loadAndGunzipFile` (activity_risk 13.49), `loadLanguage` (13.12), `initialize` (12.23), and `recognize` (11.78) — are all in the fire quadrant, meaning they are both structurally complex and were touched within the last 30 days. Each recorded 1 touch in the last 30 days and shows days_since_changed of 0, indicating a very recent push. The fifth top hotspot, the anonymous function in browser/getCore.js, is a debt-quadrant function that has not been changed in 152 days and received zero touches in the last 30 days. Active development and accumulated structural complexity are not mutually exclusive — the high activity_risk scores in this repository reflect a codebase that is being actively evolved, with some functions that have grown more complex than their current test surface can safely cover."
...
### `loadAndGunzipFile` — worker-script/index.js
...
### `loadLanguage` — worker-script/index.js
...
The file's `pr_review_comment_density` of 0.5 across only 3 total commits suggests this code has received some review attention relative to its commit volume — worth keeping in mind when planning how to pitch a refactor to maintainers.
...
### `initialize` — worker-script/index.js
...
### `recognize` — worker-script/index.js
...
`processOutput`, which `recognize` calls and which also appears in the context data as a fire-quadrant function in the same file with activity_risk 7.91, is itself being actively changed — meaning two coupled functions in the same file are both in the current development window.
...
### `<anonymous>` — worker-script/browser/getCore.js
...
Cyclomatic complexity of 15 across a function whose entire job is selecting a file path is a sign that the branching structure has outgrown its original design. The backward-compatibility shim for `TesseractCoreWASM` (an older module name) adds another conditional at the end, and the `author_count_90d` of 0.0 means no one has touched this file in the last quarter.
