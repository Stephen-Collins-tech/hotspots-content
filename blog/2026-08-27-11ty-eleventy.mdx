---
title: "eleventy's template and plugin layer carries the most structural debt"
...
The file-level signals add context without proving defect: 13 bug-linked commits and a 50% bug-fix rate on this file over its history suggest the JavaScript template engine has needed correction before. The current function complexity is a structural explanation for why that might happen again.
...
The external signals are worth noting: two-thirds of the commits to this file (a bug-fix rate of 67%) have been bug fixes, across 6 total commits. That's a small sample, but it suggests this file has required correction more often than it has been extended.
...
The `exit_heavy`, `god_function`, and `long_function` pattern flags are all accurate. The file-level signal is notable in a different way: a bug-fix rate of 100% across 2 commits means every commit to this file has been a bug fix. Small sample, but worth keeping in mind.
