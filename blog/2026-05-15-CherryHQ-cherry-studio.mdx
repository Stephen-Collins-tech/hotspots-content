---
title: "cherry-studio's AI core and MCP layer carry the highest activity risk — 2 functions to address first"
...
### `truncateOutput` — src/renderer/src/pages/home/Messages/Tools/shared/truncateOutput.ts

Located in the shared utilities for the Messages/Tools UI layer, truncateOutput most likely clips or shortens tool output strings for display in the chat interface. Its cyclomatic complexity of 5, max nesting depth of 1, and fan-out of 3 are all low — this is a watch-quadrant function, meaning it is active (touched once in the last 30 days, last changed 6 days ago) but carries no significant structural risk. The metrics reveal a well-scoped utility with minimal branching and almost no coupling.

**Recommendation:** No refactoring is warranted. Continue monitoring if display formatting requirements expand — if new truncation modes or content-type handling are added, CC will climb quickly in a utility this small and a follow-up extraction will be straightforward.
