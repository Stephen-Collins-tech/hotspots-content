---
title: "restic's repository layer carries the highest activity risk — 5 functions to address first"
...
Across restic's 2,049 functions, 259 score in the critical band — and four of the five highest-risk entries are in the 'fire' quadrant, meaning they combine high structural complexity with commits in the last 30 days. `snapshotPath` in `internal/fs/fs_local_vss.go` leads the list with an activity_risk of 15.97, touched twice in the last 30 days, carrying a fan-out of 24 and a max nesting depth of 7. That is not a cleanup item for later — it is a live regression surface. I would start my review there, then move immediately to `Rewrite` in the master index, which pairs a fan-out of 50 with an activity_risk of 15.48.
...
It was last touched 9 days ago and received one commit in the last 30 days. The file's `bug_fix_fraction` is 0.5 across two commits.
...
The `selectRandomPacksByPercentage` function in the same file (`cmd_check.go`) appears in the context data as a watch-quadrant function with 2 touches in the last 30 days — indicating that the check command as a whole is under active development across multiple functions simultaneously.
