---
title: "Swiper's mousewheel and touch modules carry the highest structural risk — 5 functions to address first"
...
What makes this a structural debt risk rather than an emergency today is the 97 days of silence. No commits, no authors active in the last 90 days (`author_count_90d: 0`). The file's entire commit history amounts to a single commit, and 100% of the commits on this file are tagged as bug fixes — meaning the one time someone did work here, it was to correct a defect. That is thin institutional coverage for a function this complex.
...
Three commits have touched this file in the last 30 days, with two distinct authors. The file's `bug_fix_fraction` is 0.75, meaning three of its four total recorded commits are tagged as bug fixes. I want to be careful here: that does not prove `onTouchMove` is currently defective, but it does establish a pattern of correctness work on this file that makes the combination of CC 127 and active multi-author editing genuinely concerning. With 127 paths, adding a guard for one edge case can inadvertently disable another, and the exit-heavy pattern — multiple early returns scattered throughout the function — makes it hard to trace which guards are even active at any given call site.
