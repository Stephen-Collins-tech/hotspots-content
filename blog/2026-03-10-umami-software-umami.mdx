...
- The POST handler in src/app/api/send/route.ts has a recent commit activity of 15.36 — the highest in the project — meaning it is both the most structurally complex entry point (CC 28, fan-out 35) and the most actively changed. Prioritize characterization tests here before any other refactoring work.
...
- MetricLabel in src/components/metrics/MetricLabel.tsx has CC 41 — unusually high for a UI component — and is still actively changing (recent commit activity 14.07). Decomposing it into per-metric sub-components would reduce regression risk and make individual metric display logic independently testable.
