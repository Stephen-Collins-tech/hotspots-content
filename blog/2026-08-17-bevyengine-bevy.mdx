---
title: "bevyengine/bevy's rendering pipeline carries the highest activity risk — 5 functions to address first"
...
Across 18,642 functions in bevyengine/bevy, 293 are in the critical band — and every single one of the top five hotspots sits in the 'fire' quadrant, meaning high structural complexity combined with active commit churn right now. The top-ranked function by activity-weighted risk, `derive_as_bind_group`, carries a cyclomatic complexity of 83 and was touched twice in the last 30 days, giving it an activity-weighted risk score of 19.43. I'd start there and with `load_gltf` before any rendering sprint this week — these aren't cleanup items deferred to a backlog. They are live regression risks on code that is changing today.
