...
The function hasn't been touched in 66 days and has zero commits in the last 30 days. The `bug_fix_fraction` of 1.0 on this file reflects that the single recorded commit was a fix — worth noting as context, though one commit is too small a sample to draw conclusions about defect history.
...
The `pr_review_comment_density` of 3.875 on this file is the highest in the top five, which tells me reviewers have historically had a lot to say about it. That is a useful signal: it suggests this function's complexity has attracted scrutiny before, even if no reverts or bug-linked commits appear in the record.
...
The `pr_review_comment_density` of 3.0 is the second-highest in the top five, suggesting reviewers have had substantive feedback on this file historically. The function hasn't been touched in 66 days.
...
While the top five are all structural debt, the `context_only` data surfaces four functions in the fire quadrant — actively changing right now. `apply_generic_model_settings` in `aider/models.py` carries a cyclomatic complexity of 100 and has been touched 5 times in the last 30 days, with an activity_risk of 13.58. `configure_model_settings` in the same file has 1 touch in the same period, with an activity_risk of 10.44. Both sit in a file that is clearly under active development — the model configuration layer is being actively iterated, and that context makes the CC 100 in `apply_generic_model_settings` a live regression risk worth monitoring even though it doesn't crack the top five by activity_risk. I would flag it as next in queue after the debt functions are addressed.
