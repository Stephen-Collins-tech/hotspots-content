...
description: "In twelve repos — mostly Python and TypeScript, with one Go repo — a single scalar from each codebase's bug-fix history correctly predicted whether fine-tuning a local LLM would beat the baseline."
pubDate: 2026-05-20
...
tags:
  ...
  - "machine-learning"
draft: false
introContent: "In twelve repos — mostly Python and TypeScript, with one Go repo — a single scalar from each codebase's bug-fix history correctly predicted whether fine-tuning a local LLM would beat the baseline."
---

In twelve repos — mostly Python and TypeScript, with one Go repo — a single scalar from each codebase's bug-fix history correctly predicted whether fine-tuning a local LLM would beat the baseline. The result: 12 for 12, with a clean separation between the two groups.
...
On three other repos — Django, scikit-learn, and Vue.js — the same adapter recipe produced ρ values between +0.26 and +0.60, all well above the baseline. The spread is enormous. Something structural about the repos is determining the outcome.
...
We calibrated the screener on eight repos where we had both the screening metric and known fine-tuning outcomes, then validated on four more.
...
| vite | TypeScript | low | ✓ | RUN — correct |
...
The eight calibration repos are cleanly separable by mean defect density, with a gap between the weakest winner (react) and the strongest loser (vscode). The threshold is set as the midpoint between those two values — a mechanical rule, not a tuned constant, though with only one repo near each side of the boundary, the midpoint's stability is itself something more data would test.

We then validated on four additional TypeScript-heavy open-source repos (Excalidraw, Deno, Angular, n8n). All four were above the threshold, and all four beat the baseline — FT ρ between +0.29 and +0.62 against baselines between +0.01 and +0.32. The full set is 12 for 12.
...
**The threshold is empirical.** The midpoint between min-winner and max-loser is a sensible heuristic, but it's derived from twelve repos across two primary languages (Python and TypeScript, with one Go repo). It may shift as more repos — especially repos in other languages, with different commit cultures, or different issue-tracking practices — are added to the calibration set.
...
The screener's clean 12/12 accuracy rests on a gap between the weakest winner (react, just above the threshold) and the strongest loser (vscode, just below it). The prediction would break down if a repo were found that:
...
Neither has appeared yet in twelve repos. Both remain plausible with more data.
