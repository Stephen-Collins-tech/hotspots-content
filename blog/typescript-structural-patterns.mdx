...
| Pattern | Repos (of 28) | Avg count | Hit max (5) |
|---|---|---|---|
...
These counts are capped at 5 per repo (the number of top hotspot functions analysed). A repo at count 5 means every one of its top-5 highest-risk functions carries that pattern.
...
**`long_function`** — a function whose body has grown to the point where it cannot be read in one sitting and its full set of responsibilities cannot be held in working memory. Highly correlated with `god_function` in this corpus: 11 repos max out on both simultaneously.
