...
titleSeo: "tldraw code health: Editor.ts functions need refactoring to reduce regression risk"
introContent: "Five functions in tldraw's editor layer carry critical risk scores of 18+; three live in Editor.ts with cyclomatic complexity reaching 122, creating high regression risk during active development."
qaSection:
  - question: "What is complex_branching and why does it matter in tldraw?"
    answer: "Complex branching means deeply nested if/else chains or switch statements that make code paths hard to follow. In tldraw's editor functions, this pattern forces reviewers to trace multiple conditional paths, increasing the likelihood of missed edge cases during refactoring."
  - question: "How do I reduce cyclomatic complexity in TypeScript?"
    answer: "Extract conditional logic into separate helper functions, use early returns to flatten nesting, and replace nested ternaries with guard clauses. For example, move validation checks into a dedicated `validate()` function rather than embedding them in the main function body."
  - question: "Is tldraw actively maintained?"
    answer: "Yes — the high activity-weighted risk scores on these functions indicate they receive frequent commits. This makes their complexity especially problematic, since developers are regularly touching code that is already hard to reason about."
  - question: "How do I reproduce this analysis?"
    answer: "Run the hotspots CLI against the tldraw repository and specify the commit SHA used for this analysis to see the same risk metrics and function dependencies."
  - question: "What does activity-weighted risk mean?"
    answer: "Complexity × recent commit frequency — functions that are both hard to understand AND actively changing are the highest priority. A function with cyclomatic complexity of 100 that hasn't changed in two years poses less regression risk than one with complexity 36 that receives weekly commits."
---
