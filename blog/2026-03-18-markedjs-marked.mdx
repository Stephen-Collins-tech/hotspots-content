...
qaSection:
  ...
  - question: "Is marked actively maintained?"
    answer: "The structural data at this snapshot suggests the Tokenizer and Instance layers are in a stable phase rather than active development. The top three src/ hotspots — `list`, the anonymous Instance function, and `use` — are all in the debt quadrant, having not been touched in 33–34 days. The bin/ CLI functions (`start` and `main`) show high recent commit activity but are tooling entry points rather than core library logic. marked is a widely-used Markdown parser, and the debt in its core parsing layer warrants refactoring before the next significant development push."
