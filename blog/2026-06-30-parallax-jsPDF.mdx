---
title: "parallax/jsPDF: WebPDecoder.js carries the highest structural debt — 4 functions to address first"
...
| Rank | Function | File | Activity Risk | Band | CC | ND | FO | Touches (30d) |
|------|----------|------|---------------|------|----|----|----|---------------|
| 1 | `jsPDF` | src/jspdf.js | 20.2 | critical | 314 | 9 | 303 | 0 |
| 2 | `WebPDecoder` | src/libs/WebPDecoder.js | 20.2 | critical | 360 | 11 | 163 | 0 |
| 3 | `_WebPDecoder` | src/libs/WebPDecoder.js | 20.2 | critical | 349 | 11 | 149 | 0 |
| 4 | `rb` | src/libs/WebPDecoder.js | 19.37 | critical | 79 | 11 | 16 | 0 |
| 5 | `Le` | src/libs/WebPDecoder.js | 19.27 | critical | 58 | 10 | 18 | 0 |
...
The external signals are sparse but worth noting: `pr_review_comment_density` of 8.0 is the highest of any function in the top five, suggesting that past pull requests touching `src/jspdf.js` attracted substantial review discussion. The bug-fix fraction is 0.5 across two total commits — not alarming, but consistent with a function that is hard to change correctly.
...
This function hasn't been touched in 117 days, and `author_count_90d` is 0.0. The sole historical commit was a bug fix. There is no recent ownership.
...
Like `WebPDecoder`, it has been untouched for 117 days, has zero recent authors, and its only historical commit was a bug fix.
