---
title: "textgen's chat and API layer carries the highest activity risk — 3 functions to address first"
...
The headline risk in oobabooga/textgen sits squarely in two files: `modules/chat.py` and `modules/api/completions.py`. The top-ranked function, `generate_chat_prompt`, carries an activity_risk score of 21.02 with a recent activity score of 19.28 — meaning it is both structurally extreme (cyclomatic complexity of 152) and actively changing right now, making it a live regression risk rather than a backlog cleanup item. Across 1,248 total functions, 175 are rated critical, and the top four non-vendored hotspots all share the "fire" quadrant, indicating simultaneous structural complexity and ongoing commit pressure.
