---
title: "minimind's RL trainers carry the highest activity risk — 4 functions to address first"
...
introContent: "minimind's RL training layer is both its most structurally complex and most actively changing code — two trainer functions sit at CC 52 with commits landing days ago, making refactoring a live risk, n"
pubDate: 2026-05-11
...
### `process_assistant_content` — scripts/web_demo.py

From its name and location in the web demo script, this function likely parses or formats assistant-generated content for display in the demo interface — potentially handling multiple output formats, streaming tokens, or special markup. Despite its relatively moderate cyclomatic complexity of 22, the exit_heavy pattern flags that it contains multiple return or exit paths, each of which represents a distinct test case and a potential gap in coverage. It is in the debt quadrant with 0 touches in the last 30 days and has been unchanged for 42 days, meaning it is structural debt that will be expensive to modify safely when the demo layer is next updated.

**Recommendation:** Map out each exit path explicitly — there are enough to make the function's full behavior non-obvious — and cover each with a test before the next modification. If any exit paths handle overlapping conditions, consolidate them to reduce the branch count and make the single-exit flow easier to follow.
