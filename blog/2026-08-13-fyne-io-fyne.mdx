...
The file-level history adds context without being alarming: 14 total commits on `window.go`, a bug-fix rate of 14.3%, and 4 distinct authors in the last 90 days. That author spread combined with the current commit rate means multiple engineers are reasoning about this function concurrently — a coordination risk on top of the structural one.
