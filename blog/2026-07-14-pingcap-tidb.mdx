---
title: "pingcap/tidb's planner and DDL layer carries the highest activity risk — 5 functions to address first"
titleSeo: "tidb code health: planner and DDL hotspots drive refactoring priority in pingcap/tidb"
description: "Analysis of pingcap/tidb at commit 5d6fdbe finds five critical-band functions in the query planner, DDL, and expression layers that are both structurally complex and actively changing, with activity_risk scores between 18.85 and 19.53."
...
The external signals show a `bug_fix_fraction` of 0.67 across the 3 recent commits touching this file, and a `pr_review_comment_density` of 0.67 — the highest of any file in the top five. That review activity suggests the team already recognizes this function warrants scrutiny.
...
This file has a `bug_fix_fraction` of 1.0 across its single recent commit — 100% of the recorded changes to this file have been bug fixes. That is historical context for the file, not proof this function is defective, but it does strengthen the case for prioritized review.
...
Two commits in the last 30 days and a last-modified date of 4 days ago place this firmly in active territory. The `bug_fix_fraction` of 0.5 across those two commits indicates one fix and one other change — moderate historical signal.
...
This file's `bug_fix_fraction` is 1.0 on its single recent commit. Expression folding bugs are particularly hard to detect because they produce incorrect results only on specific constant-expression shapes.
