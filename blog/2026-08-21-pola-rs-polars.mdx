...
At commit 9547025, polars has 21,718 analyzed functions, 937 of which are in the critical band — and all five of the top hotspots are in the 'fire' quadrant, meaning they are both structurally complex and actively changing right now. The top-ranked function, `lower_ir`, carries an activity_risk of 21.18 against a cyclomatic complexity of 80 and was last modified two days ago. I would start there, but the projection-pushdown optimizer's `pushdown` function — with a cyclomatic complexity of 215 — is the single most structurally demanding function in this set and deserves immediate parallel attention.
...
The `pr_review_comment_density` of 5.0 on a file with only one total commit is the highest review comment density in the top five. That one commit attracted significant reviewer attention, consistent with a function this complex.
...
The file-level `bug_fix_fraction` of 1.0 means every commit recorded against this file has been classified as a bug fix. With only one total commit that is a small sample, but the only recorded change to this file was corrective.
...
Two authors have touched this file in the last 90 days and the `pr_review_comment_density` is 1.5, suggesting active collaboration on a function where shared mental models are already difficult to maintain at CC 74.
...
The `bug_fix_fraction` of 1.0 on a single commit matches the pattern seen in the credential provider file: the only recorded change was corrective. Two separate I/O-layer files showing the same pattern is worth noting as historical context, even with a small sample.
