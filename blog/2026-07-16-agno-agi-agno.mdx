---
title: "agno-agi/agno's async run layer carries the highest activity risk — 5 functions to address first"
titleSeo: "agno-agi/agno code health: async streaming run functions lead refactoring priority in 2026"
description: "Five god-functions across agno's agent, team, and workflow async streaming layer share activity_risk scores above 21, cyclomatic complexity between 129 and 261, and were each touched within the last 24 hours."
...
qaSection:
  ...
  - question: "What does activity-weighted risk mean?"
    answer: "Activity-weighted risk combines structural complexity — derived from cyclomatic complexity, nesting depth, and fan-out — with recent commit frequency, so functions that are both hard to understand and actively changing score the highest. A function with a cyclomatic complexity of 200 that has not been touched in two years scores much lower than one with a cyclomatic complexity of 50 that is committed to every day, because the complex-but-dormant function presents lower near-term regression risk. In agno's case, the top five functions each carry an activity_risk above 21 because they are structurally extreme and were each touched within the last 24 hours — that is the combination that makes them a live concern rather than a backlog item. The goal of the metric is to direct refactoring effort toward functions where complexity is most likely to introduce a bug in the current development cycle."
---

The five functions that dominate agno's risk profile are all in the 'fire' quadrant: structurally extreme and actively changing right now. The top-ranked function, `_acontinue_run_stream` in `libs/agno/agno/team/_run.py`, carries an activity_risk of 21.44 with a cyclomatic complexity of 227 and was last modified one day ago — that is a live regression surface, not cleanup debt. Agno is an agent framework with 11,361 analyzed functions, of which 2,738 (roughly 24%) score in the critical band; I would start review and refactoring work in the async run layer immediately, because that is where complexity and development velocity are colliding in real time.
...
The context-only functions — `generate_id` in `utils/string.py`, `select` in `workflow/types.py`, and the logging helpers in `utils/log.py` — are all in the watch quadrant with low structural complexity. They are active but not a refactoring priority; I would revisit them only if their activity_risk rises as the codebase grows.
