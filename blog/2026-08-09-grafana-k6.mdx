---
title: "grafana/k6's browser test static assets carry the highest activity risk — 5 functions to address first"
...
The file's `bug_fix_fraction` is 1.0 across its recorded commit history, meaning every commit to this file has been tagged as a bug fix. That's historical context, not proof of a current defect, but it does suggest this code has needed correction before.
...
The `context_only` data includes several Go functions from the actual k6 codebase — `applySlowMo` in `browser/common/hooks.go`, `mapRequestEvent` in `browser/browser/request_mapping.go`, and `mapResponseEvent` in `browser/browser/response_mapping.go` — all in the moderate/watch band with low structural complexity and 2 touches in the last 30 days. None of these warrant refactoring attention now, but `mapRequestEvent` and `mapResponseEvent` are worth monitoring: event-mapping functions in a browser automation context tend to grow as new event types are added, and watching their complexity trend over time is worthwhile. The `throw` function in `internal/js/modules/k6/experimental/streams/errors.go` carries the `experimental_` path signal — an actively-touched error-handling utility in an experimental module is something I'd keep an eye on as that API stabilises.
