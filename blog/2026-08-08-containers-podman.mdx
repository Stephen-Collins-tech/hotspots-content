...
The function lives in the same file as `setupContainer` and shares its git history — last changed 3 days ago, three authors in 90 days, a bug-fix rate of 33%. The fan-out of 50 means teardown touches as many subsystems as setup, which makes sense architecturally but compounds the review burden. At CC 19 and ND 5, the function is already past the threshold where any individual engineer can hold all its paths in working memory simultaneously.
