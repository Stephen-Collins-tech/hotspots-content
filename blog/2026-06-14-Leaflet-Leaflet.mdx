---
title: "Leaflet's GeoJSON and map core carry the highest activity risk — 5 to address first"
...
Across 956 functions analyzed at commit ff38fe5, Leaflet carries 20 critical-band functions — and the two that concern me most are `geometryToLayer` in `src/layer/GeoJSON.js` and `_findEventTargets` in `src/map/Map.js`, both in the 'fire' quadrant: structurally complex and actively changing today. `_findEventTargets` has been touched 4 times in the last 30 days against a cyclomatic complexity of 20, and `src/map/Map.js` has accumulated 15 bug-linked commits historically with a bug-fix rate of 50% — that combination argues for treating it as a live regression surface, not a cleanup item for the next sprint. The headline activity-weighted risk score of 13.53 for `geometryToLayer` (touched once in 30 days, CC 27) makes it the top real-code priority once the vendored documentation asset is set aside.
...
See more analyses with these patterns: [`complex_branching`](https://hotspots.dev/analyses/patterns/complex_branching/), [`deeply_nested`](https://hotspots.dev/analyses/patterns/deeply_nested/), [`exit_heavy`](https://hotspots.dev/analyses/patterns/exit_heavy/), [`god_function`](https://hotspots.dev/analyses/patterns/god_function/).
