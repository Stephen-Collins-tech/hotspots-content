---
title: "Scrapling's spider and stealth engines carry the highest activity risk — 4 functions to address first"
...
Scrapling's highest-priority refactoring targets sit inside two files — scrapling/spiders/engine.py and scrapling/engines/_browsers/_stealth.py — where CC scores of 41 and 42, fan-out values above 25, and recent commit activity combine to create live regression risk right now. The top-ranked function, crawl, carries an activity risk score of 18.57 and a recent activity score of 18.05, meaning it is both structurally complex and actively changing — this is not a backlog cleanup item. Across 391 total functions, Scrapling has 36 rated critical and 32 in the 'fire' quadrant, signaling that the engine and browser abstraction layers deserve immediate attention.
...
### `find_all` — scrapling/parser.py

find_all in scrapling/parser.py is almost certainly the primary DOM query and element-matching function — a core API surface that callers throughout the library depend on. Its CC of 51 is the highest in the dataset, representing 51 independent execution paths, yet its max nesting depth of only 3 suggests the complexity is driven by wide branching rather than deep nesting — likely many selector strategies or fallback patterns evaluated in sequence. With a fan-out of 29 and zero commits in the last 75 days, this is textbook structural debt: high blast radius, no recent protective testing, and overdue for refactoring before the next development push.

**Recommendation:** Given the fan-out of 29 and CC of 51, prioritize writing a comprehensive regression test suite for find_all before touching it; then use extract-method refactoring to break out each selector strategy or dispatch branch into a named, independently testable function.
...
- find_all in scrapling/parser.py has a cyclomatic complexity of 51 and has not been changed in 75 days — it is the highest-CC function in the dataset and represents the largest structural debt item in the codebase, warranting planned extract-method refactoring before the next parser development push.
