---
title: "validator.js's string validators carry the highest activity risk — 5 functions to address first"
...
qaSection:
  ...
  - question: "Is validator.js actively maintained?"
    answer: "Yes, but the activity is concentrated: the isTaxID.js checks (dkDkCheck, lvLvCheck, plPlCheck) all show touches_30d of 1 and were last changed 15 to 17 days ago — real, recent development. The five functions in my top-5 table tell a different story — all show touches_30d of 0, with isMobilePhone the most dormant at 784 days since its last change. Active development and structural debt coexist here: the tax-ID validators are getting attention while the older, larger URL and email validators sit untouched."
...
normalizeEmail has the deepest nesting of any function in the top five — a max nesting depth of 6 — which lines up with the source: a long if/else-if chain branching on domain (gmail, icloud, outlook.com, yahoo, yandex), each branch with its own nested option checks for subaddress removal and lowercasing. It's tagged `complex_branching` and `deeply_nested` together, the strongest signal in this dataset that the control flow itself is the problem, not just the line count. It's been dormant for 720 days, but all three of its recorded commits were bug fixes — a bug_fix_fraction of 1.0. That's a small sample, but it's a 100% hit rate on a function that only gets touched to fix something. The cleanest fix here is decompose-conditional: turn the five domain-specific branches into a lookup table of normalizer functions (`normalizeGmail`, `normalizeIcloud`, etc.) keyed by domain, so adding or fixing a provider's rules doesn't require re-reading the whole chain.
