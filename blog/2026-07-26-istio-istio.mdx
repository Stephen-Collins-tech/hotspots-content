---
title: "istio/istio's control plane carries the highest activity risk — 5 functions to address first"
...
At commit 1ef2d93, istio/istio has 9,975 analyzed functions, 1,016 of which fall in the critical band — and every single one of the top five hotspots is in the 'fire' quadrant, meaning they are both structurally complex and actively changing right now. The top-ranked function, `delete` in `pilot/pkg/config/kube/crdclient/types.gen.go`, carries an activity_risk of 18.46 and was last touched 7 days ago. That is not a cleanup backlog item — it is a live regression surface. I would start triage there and work down the list before any further changes land in pilot or istioctl.
...
### `buildWaypointInternal` — listener_waypoint.go
...
### `patchHTTPFilters` — listener_patch.go
...
### `BackendTLSPolicyCollection` — backend_policies.go
