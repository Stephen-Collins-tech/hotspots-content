---
title: "Spacedrive's indexing and filesystem core carries the highest structural debt — 5 functions to address first"
...
qaSection:
  ...
  - question: "Is spacedrive actively maintained?"
    answer: "Yes — the fire-quadrant data confirms active development. `MediaMetadataCard`, `bridge_daemon_events`, `ExplorerProvider`, `navigationReducer`, and `SidecarItem` all had at least one touch in the last 30 days, and `ExplorerProvider`, `navigationReducer`, `bridge_daemon_events`, `MediaMetadataCard`, and `SidecarItem` each show days_since_changed of 0. The top five hotspots tell a different story: every one of them was last modified 105 days ago, with zero touches in the past 30 days. Active development and accumulated structural debt aren't mutually exclusive — the project is clearly moving in some areas while foundational functions like `run_job_phases` (activity_risk 18.76) and `parse_apfs_list_output` (activity_risk 18.31) have been structurally complex and dormant for months."
