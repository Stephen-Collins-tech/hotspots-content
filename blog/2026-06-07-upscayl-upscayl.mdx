...
At commit a00d55f, upscayl's riskiest function — an anonymous handler in `electron/commands/custom-models-select.ts` — carries an activity_risk of 9.94 and has not been modified in 391 days, with zero commits in the past 30 days. Across 140 functions analyzed, 3 are critical and all 12 debt-quadrant functions share the same profile: structurally non-trivial code that has been left untouched long enough that contextual knowledge has almost certainly faded. I would start with `customModelsSelect` and `SettingsTab` because they sit at the boundary between the Electron main process and the renderer, exactly where state mismatches and platform-specific edge cases tend to bite.
...
Every function in the top five is a debt-quadrant entry: high structural complexity, zero recent activity, and no author activity in the past 90 days according to the `author_count_90d` signal on each file. That combination is a specific kind of risk — not a fire drill, but a trap that springs the moment someone has to make a change. The structural weight is already loaded; the blast radius is just waiting for a trigger.
