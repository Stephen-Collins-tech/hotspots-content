---
title: "seaweedfs's storage and Kafka layers carry the highest activity risk — 5 functions to address first"
...
Beyond the top five, the `context_only` functions — including `writeJSON` in `weed/s3api/iceberg/utils.go` and the Avro schema converters in `weed/mq/kafka/schema/avro_decoder.go` — are all in the "watch" quadrant with low structural complexity and cyclomatic complexity of 3–4. They are active but not structurally risky; I would not prioritize them for refactoring.
